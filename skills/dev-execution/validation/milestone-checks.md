...
### Findings Triage (Route 7)

- [ ] **Route 7 triage run** — no findings missing required frontmatter
...
If the output reports files under `.claude/findings/` with missing or invalid frontmatter, warn
the user and request that the creating agent add the required stub before the phase is marked
complete. See [.claude/skills/dev-execution/modes/phase-execution.md] → "Agent Findings
Frontmatter" for the required stub and the findings promotion pipeline.
