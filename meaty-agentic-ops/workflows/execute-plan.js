...
const KNOWN_AGENT_TYPES = new Set([
  'agent-expert', 'ai-artifacts-engineer', 'ai-engineer', 'aos-adp-reconciler',
  'aos-context-librarian', 'aos-docket-gatherer', 'aos-landing-steward', 'aos-merge-steward',
  'aos-pr-reviewer', 'aos-retro-synth', 'api-documenter', 'artifact-tracker',
  'artifact-validator', 'backend-architect', 'backend-typescript-architect',
  'bob-delegate-executor', 'changelog-generator', 'code-reviewer', 'codebase-explorer',
  'codex-executor', 'command-creator', 'data-layer-expert', 'devops-architect',
  'documentation-complex', 'documentation-expert', 'documentation-planner',
  'documentation-writer', 'feature-planner', 'feature-sprint-executor', 'frontend-architect',
  'gemini-executor', 'gemini-orchestrator', 'ica-executor', 'implementation-planner', 'karen',
  'lead-architect', 'lead-pm', 'nextjs-architecture-expert', 'openapi-expert', 'phase-owner',
  'prd-writer', 'prompt-engineer', 'python-backend-engineer', 'refactoring-expert',
  'research-technical-spike', 'search-specialist', 'senior-code-reviewer', 'spike-writer',
  'symbols-engineer', 'system-architect', 'task-completion-validator',
  'task-decomposition-expert', 'technical-writer', 'ui-designer', 'ui-engineer',
  'ui-engineer-enhanced',
  // Builtin harness agent type — has no file under .claude/agents/.
  'general-purpose',
])
...
async function inlineDegradedCouncil(p, taskOut) {
  ...
  if (validOutputs.length === 0) {
    ...
    return withRouting({ status: 'needs_opus', reason: 'inline_council_reviewers_failed', council_mode: 'inline_degraded', report: [], run_placement: placementFacts(graph) })
  }
  ...
  if (!adjudicated) {
    ...
    return withRouting({ status: 'needs_opus', reason: 'inline_council_adjudication_failed', council_mode: 'inline_degraded', report: [], run_placement: placementFacts(graph) })
  }
  ...
  if (!finalVerdict) {
    ...
    return withRouting({ status: 'needs_opus', reason: 'inline_council_final_verdict_failed', council_mode: 'inline_degraded', report: [], run_placement: placementFacts(graph) })
  }
