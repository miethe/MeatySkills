...
## v2.2 — 2026-08-06 — A milestone is what a plan is written in, not what one agent is handed

The authoring half of dispatch-time leg contracts. Execution half: `dev-execution` v1.5, rule 6 +
§ "Leg scoping" in `dev-execution/references/execution-doctrine.md`. Evidence:
`docs/project_plans/reports/workflow-v41-delegate-retro-2026-08-06.md`.

- **Rule 2 gets a scope caveat, not a rollback.** "Hand an executor a whole milestone" is an
  *authoring* claim about the unit a plan is written in and a reviewer judges — it was read as
  "one milestone = one dispatch", which put a whole C3 milestone on one leg at ~1.5–2.0× the
  `gate-risk-classes.md` §4 implementer band for its scope. A mid-tier executor receives a leg
  contract; a milestone spanning more than one file-ownership boundary is split at dispatch.
  (Originally reported here as a "20× overrun" — a unit error against a resident-context-window
  figure, corrected in `dev-execution` v1.5.1; the scoping failure is unchanged.)
- **§ "Model-conditional expansion" gains the leg contract as the same kind of render.** Under thin
  plans delegates did not build the wrong feature (zero AARs) — they shipped tests that exercised a
  path production never takes. So the granularity a mid-tier executor needs is precise files,
  interfaces, field names and a named verification path, resolved against the tree at dispatch.
- **Which makes plans thinner, not thicker.** Pre-writing leg contracts into the plan re-imports
  plan mass (rule 4) and pins names that may not survive to dispatch. Stated explicitly so the
  finding cannot be read as a licence to re-prescribe.

Origin: `node_01KZC04VHJVB70G6G9QWAVH2H1` (retro R1).
