...
2. **Output location**: `docs/project_plans/feature_contracts/[feature-slug].md`
...
**Mandatory reviewer gate (the base gate — one lens)**: `task-completion-validator` per phase; one
`karen` final-tree pass at end of feature.

**Classify each milestone for a second lens.** A second lens (`security`) is added only where the
milestone's surface matches one of three triggers — it **parses untrusted input**, it **is an
authorization/identity boundary**, or its effect is **irreversible or leaves the system**. Where one
matches, record it on that `wave_plan.phases[]` entry:

```yaml
gate_lens: [security, validator]      # or [security] where the validator adds nothing distinct
gate_lens_reason: authz-boundary      # untrusted-input | authz-boundary | irreversible-outward
```

Most milestones match none of the three and carry `gate_lens: [validator]` — that is the expected
outcome, not an under-specification. A two-lens milestone with **no** `gate_lens_reason` is a
classification error. Ruleset: `.claude/skills/dev-execution/references/gate-risk-classes.md` §2.
The optimizer pass (`dev-execution/modes/plan-optimization.md`) can do this classification for you at
the plan/execute boundary; authoring it here is better, because the plan then carries its own gate plan.
...
- **`karen` at each plan-milestone boundary — only when `context_class` is C3 or C4.** The plan's `wave_plan.phases[]` entries ARE those boundaries. ("Plan milestone" = a reviewable state of the system, per `plan-doctrine.md` rule 2 — not the older within-phase "batch checkpoint" sense.) C3/C4 are the classes whose posture already calls for operator checkpoints at every milestone boundary; **C1/C2 Tier 3 plans get the single `karen` final-tree pass and nothing more.** Per-milestone karen on a C1/C2 plan duplicates that final pass — `gate-risk-classes.md` § Karen placement.
