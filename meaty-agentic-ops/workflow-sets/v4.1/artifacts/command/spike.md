...
### Next Actions Table (standard close)

End the response with the **Next Actions table** — spec: [.claude/skills/dev-execution/references/next-actions-table.md]. Emit a row per unresolved open question that must be answered before the parent work proceeds (Target = the `*-open-questions.md` path), and — in `--leg-of` mode — the return-to-parent handoff row (back to `/plan:explore` synthesis or the PRD author). Empty state when the spike fully resolved its charter and nothing is blocked.
