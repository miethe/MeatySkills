...
# --target atlas|intenttree|skillmeat|meatywiki|ccdash. Recurring routes (phase/program/
# readiness) require --instance-key when the target is atlas or intenttree (D1) — see
# references/aos-integration.md §1.1.
"$DELIVERY_REPORT_PYTHON" "$DELIVERY_REPORT_SKILL_DIR/scripts/delivery_report.py" export \
  ...
  --target atlas --out .claude/reports/<slug>/writeback.json
```
...
6. **Attach and (optionally) export.** Record the HTML path in the closeout / tracker; emit a
   writeback envelope for an AOS subsystem with `export` (`references/aos-integration.md`). The
   `atlas` target is the host+link path: `hooks/publish-report.sh` (in `dev-execution`, M3) ingests
   the exported HTML into the atlas capsule host and links the resulting preview URL onto the bound
   IntentTree node. **Only `dossier` fires this automatically today** (wired at
   `plan-execution.md` §8, after the dossier render). The `phase` route is wired but dormant — it is
   a true no-op until something supplies `$PHASE_REPORT_MANIFEST` (see `dev-execution/SKILL.md` §
   dossier/publish hooks). `feature`/`program`/`readiness` have no close-hook wiring at all yet; the
   engine (`scripts/publish_report.py`) is manually invocable for any route in the meantime.
...
- **Writeback ingestion** — `export` itself still only emits an envelope; do not treat *export* as
  having written anywhere. For `atlas`/`intenttree`, actuation now exists as a separate step
  (`scripts/publish_report.py`, invoked by `hooks/publish-report.sh` — see `references/
  aos-integration.md` §1) — do not conflate the two: `export` stays pure/offline, actuation is
  where the network/subprocess calls live. `skillmeat`/`meatywiki`/`ccdash` targets remain
  envelope-only with no actuator (unchanged).
- **IntentTree write-back of item state** — the link actuator writes an `ExternalLink` pointer
  (the report's URL) onto a node; it never writes node status/completion.
