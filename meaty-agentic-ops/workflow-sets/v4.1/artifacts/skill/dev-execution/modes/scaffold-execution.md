...
```
Task("task-completion-validator", "Scaffold review: ${feature}.
Layers scaffolded: schema/DTO, repository, service, API, frontend, tests.
Files changed: ${files_changed}
Verify: each layer is actually WIRED to the next (not just present), the API is reachable from the
real entry point, and the tests exercise it rather than asserting the scaffold's shape.")
```

Runs in `plan` permissionMode (edit-less). Verdict: `APPROVED` or `CHANGES_REQUESTED`.
