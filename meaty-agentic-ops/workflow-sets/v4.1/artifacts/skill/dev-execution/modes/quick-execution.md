...
```
Task("task-completion-validator", "Tier 0 review: ${feature-slug}.
Acceptance criteria (inline): ${ac_list}
Files changed: ${files_changed}
Verify each AC is met, tests exist and pass, and no regression was introduced.")
```

Runs in `plan` permissionMode (edit-less). Verdict: `APPROVED` or `CHANGES_REQUESTED`. Do not commit
