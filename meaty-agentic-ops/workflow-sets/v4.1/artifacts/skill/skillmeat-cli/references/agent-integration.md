...
This guide teaches agents how to integrate with the skillmeat-cli skill to discover, recommend, and deploy artifacts that enhance development workflows.

## AOS / Codex Projection Note

For AOS context injection, prefer `op context pack`; it uses SkillMeat project memory as Part 4 on a
best-effort basis and keeps the bundle assembly model-free.

For Codex harness projection, the current seam is narrower:

- Per-artifact Codex overlay works on the SkillMeat development branch:
  `skillmeat deploy <artifact> --profile codex --apply-overlay --project <repo> --type <type>`.
- Bundle-level Codex overlay is **not** available yet: `skillmeat bundle deploy` does not accept
  `--profile` / `--apply-overlay`, and `deploy <bundle-name>` does not expand bundle members.
- Do not hand-roll a 200+ member bundle projection loop in the launchpad. Keep using `.claude` as the
  canonical projection until SkillMeat ships bundle-level overlay support and the release reaches
  the consumed branch.

Reference: `docs/project_plans/exploration/codex-aos-integration/skillmeat-deploy-profile-handoff.md`.
...
5. DEPLOY: Execute with permission
   ...
   # Codex per-artifact only, when available:
   skillmeat deploy <name> --type <type> --profile codex --apply-overlay --project <repo>
