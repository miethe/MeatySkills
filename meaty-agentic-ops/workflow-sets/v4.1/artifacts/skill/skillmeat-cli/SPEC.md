...
- Orchestration scripts (Claude Code Dynamic Workflows, flat `.js/.ts/.py/.sh`): add + deploy (`add orchestration <file>`, `deploy`) — distinct from the `workflow` type
- Workflow artifact management: update, delete (`workflow update|delete`) — SkillMeat's own directory+manifest workflow type, **not** Claude Code Dynamic Workflows
...
| Add or deploy a Claude Code Dynamic Workflow script (`orchestration` type, `.claude/workflows/*.js`) | `workflows/deployment-workflow.md § "Claude Code Dynamic Workflows (ORCHESTRATION)"` | `docs/user/guides/cli/commands.md § "Deployment"` (`add orchestration`) |
...
| Update or delete a SkillMeat `workflow` artifact — directory+manifest type, **not** a Claude Code Dynamic Workflow (`workflow update\|delete`) | `workflows/management-workflow.md` | `docs/user/guides/cli/commands.md § "Core Commands"` |
