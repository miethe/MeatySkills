...
> [`../dev-execution/git-worktree-pr-protocol.md`](../dev-execution/git-worktree-pr-protocol.md). Set
> per-phase `model` / `effort` in `wave_plan.phases[]` per
> [`docs/agentic-operator/MODEL-ROUTING.md`](../../../docs/agentic-operator/MODEL-ROUTING.md): default
> **Sonnet 5** (`claude-sonnet-5`) for implementation phases, **Opus 5** (`claude-opus-5`) for architecture /
> integration / cross-wave-merge / spine phases (flagship as of 2026-07-24; Opus 4.8 now legacy), and `xhigh`
> effort for the hardest phases. Bounded, contract-clear phases may offload to **ICA Sonnet 5**
> (`claude-sonnet-5[1m]`, free-to-us) behind the reviewer gate; route cheap/mechanical high-volume fan-out to
> **Sonnet 4.6** (`claude-sonnet-4-6[1m]`) / **Haiku** for token economy (Sonnet 5's new tokenizer emits ~30%
> more tokens — MODEL-ROUTING §2). The "sonnet executor" cells in the table above resolve to this policy.
...
5. **Model Assignment**
   - Evaluate each task for the appropriate model:
     ...
     - Route UI wireframing/design → `gemini-3.5-flash`
     - Route web research → `gemini-3.5-flash`
   ...
   - Assign effort levels per model using the **Canonical Effort Vocabulary** in `./references/multi-model-guidance.md`. Effort is a model-keyed reasoning budget, not a size estimate — use the `Estimate` column for story points or hours.
     ...
     - gemini (3.5-flash/3.1-pro-preview): `none` | `low` | `medium` | `high`
   ...
   - **Orchestrator model**: Set a plan-level `wave_plan.orchestrator_model` default (the orchestration-loop model); add `phases[].orchestrator_model` overrides only for Tier 3 / large Tier 2 phases with high novelty or open OQs. See `references/multi-model-guidance.md` § Orchestrator Model Escalation.
...
5.5. **Required Artifacts Resolution**
   - **Enumerate.** From the phase agent-routing (step 4) and Model Assignment (step 5) above, plus
     any MCP/workflow/context the plan needs, build the `required_artifacts` list — plan-level
     frontmatter and/or per-phase `wave_plan.phases[].required_artifacts`. Schema:
     `./references/plan-frontmatter-schema.md` §5.7.
   ...
   - **Route every non-`available` entry.** Mirror the External Model Pre-Work Batching `batch_0`
     pattern above: insert an explicit `batch_0` authoring/enhancement task *before* the phase that
     consumes it, or — if it can't be resolved in-plan — a named blocker that halts the execute
   ...
   **Execute (Tier 2/3 handoff)** — once the plan file is written, model routing (including
   `orchestrator_model`) is finalized, and every `required_artifacts` entry is `available` or covered
   by a `batch_0` task / named blocker (step 5.5), emit the execution-command handoff so the operator
   knows which orchestrator drives the run:
   ...
   The plan-default is `wave_plan.orchestrator_model`; list per-phase overrides only when one or more `phases[].orchestrator_model` are set. If neither is set, name the operator's session default (typically Opus 5).
