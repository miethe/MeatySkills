...
**Model Routing Notes**:
- [Plan-level `orchestrator_model` default (the orchestration-loop model, e.g. `opus-5`) + any per-phase `orchestrator_model` overrides with rationale — distinct from the delegate/implementer model in the table above; escalate a phase's orchestrator sparingly (Tier 3 / large Tier 2, high novelty or open OQs)]
...
- **Section 6 (Model Routing)**: Propagate into task-level model/effort decisions in the plan's task table, and propagate the plan-level `orchestrator_model` default + any per-phase `orchestrator_model` overrides into `wave_plan`.
