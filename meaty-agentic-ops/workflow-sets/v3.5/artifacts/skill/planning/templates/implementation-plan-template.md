...
| 5 | UI Layer | X pts | ui-engineer-enhanced, frontend-developer, ui-designer | sonnet (+ gemini-3.5-flash for design) | External model for UI-001 wireframing |
...
- Mixed phases: list the primary Claude model plus any external model in parens (e.g., `sonnet (+ gemini-3.5-flash for UI-001)`).
...
- `Model` — Assigned executor: `opus` | `sonnet` | `haiku` | `gpt-5.6-terra` | `gemini-3.5-flash` | `gemini-3.1-pro-preview` | `nano-banana-pro`
...
| Task ID | Task Name | Description | Acceptance Criteria | Estimate | Subagent(s) | Model | Effort | Dependencies |
|---------|-----------|-------------|-------------------|----------|-------------|-------|--------|--------------|
| UI-001 | Component Design | Design/update UI components | Components support all states | 3 pts | ui-designer | gemini-3.5-flash | medium | API-004 |
...
- **`haiku`** (Claude Haiku 4.5) - Default for documentation, exploration, simple queries, and mechanical tasks
- **`gemini-3.5-flash`** - For design/wireframing, web research, complex UI workflows, multi-modal analysis
...
| gemini-3.5-flash / gemini-3.1-pro-preview | `none`, `low`, `medium`, `high` | `medium` |
