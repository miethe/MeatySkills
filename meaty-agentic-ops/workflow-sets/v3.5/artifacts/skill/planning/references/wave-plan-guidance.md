...
```yaml
wave_plan:
  orchestrator_model: opus-5   # Optional. Plan-wide default orchestrator (session/main-loop) model; cascades to all phases.
  ...
  phases:
    - id: P1                  # Required. Short label matching the phase number (P1, P2, …)
      ...
      orchestrator_model: fable-5   # Optional. Overrides the plan-level orchestrator_model for THIS phase only (Tier 3 / large Tier 2).
...
|-------|------|----------|---------|-----------|
| `orchestrator_model` | `string` | Optional | — | Plan-wide default orchestrator (session/main-loop) model — the model that drives the orchestration loop for the plan. Distinct from `phases[].model` (the delegate/implementer model). Registry short form (e.g. `opus-5`, `fable-5`, `sonnet-5`). Cascades to all phases unless a phase sets its own `orchestrator_model`. Absent = operator's session default (typically Opus 5). |
...
| `phases[].model` | `string` | Optional | — | Default model for this phase's implementer dispatches. Values: `opus` \| `sonnet` \| `haiku` \| `gpt-5.6-terra` \| `gemini-3.5-flash` \| `gemini-3.1-pro-preview` \| `nano-banana-pro`. Per-task `Model` column overrides this default. |
| `phases[].orchestrator_model` | `string` | Optional | — | Overrides the plan-level `orchestrator_model` for THIS phase only — the orchestration-loop model, distinct from `phases[].model` (the delegate/implementer model). Registry short form (e.g. `fable-5`, `opus-5`). Use sparingly — generally Tier 3 / large Tier 2 phases with high algorithmic novelty or open architectural questions. |
...
**Orchestrator model precedence**: The orchestrator model — the session / main-loop model that drives the orchestration loop — follows its own cascade, first match wins downward:

1. **Plan level** — `wave_plan.orchestrator_model` is the plan-wide default and the common case; most plans set only this.
2. **Phase level** — `wave_plan.phases[].orchestrator_model` overrides the plan default for that phase only. Escalate sparingly — generally only Tier 3 or large Tier 2 phases with high algorithmic novelty or open architectural questions where a wrong foundational decision cascades irrecoverably.
3. **Session default** — if unset at both levels, the orchestrator model is the operator's session default (typically Opus 5).

This is DISTINCT from the delegate `model` axis (`phases[].model` / per-task `Model`): `orchestrator_model` names who orchestrates the loop, while `model` names who implements inside a phase's Task() dispatches. See `references/multi-model-guidance.md` § Orchestrator Model Escalation for when to escalate a phase's orchestrator model.
