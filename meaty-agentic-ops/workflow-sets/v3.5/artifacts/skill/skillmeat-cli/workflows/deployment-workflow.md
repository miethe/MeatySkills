...
> **Artifact types deploy the same way.** `skill`, `command`, `agent`, `hook`, `mcp`,
> **`orchestration`**, and `workflow` all follow this add → deploy flow. If you're deploying a
> Claude Code Dynamic Workflow script (`.claude/workflows/*.js`), jump to
> § "Claude Code Dynamic Workflows (ORCHESTRATION)" below first — it is added with a dedicated
> subcommand and is a distinct type from SkillMeat's `workflow`.
...
## Claude Code Dynamic Workflows (ORCHESTRATION)

Claude Code Dynamic Workflow definition files — the flat scripts the `Workflow` tool runs
(`.claude/workflows/execute-plan.js`, `review-council.js`, `auto-feature.js`, …) — are
**deployable artifacts of type `orchestration`**. Common failure mode: an agent assumes these
are "SkillMeat-internal" or conflates them with SkillMeat's `workflow` type and skips them.
They are neither internal nor the same type — deploy them like any artifact.

**ORCHESTRATION vs WORKFLOW**

| | `orchestration` | `workflow` |
|---|---|---|
| Artifact | Claude Code Dynamic Workflow script | SkillMeat multi-stage execution process |
| Shape | flat file (`.js` / `.ts` / `.py` / `.sh`) | directory + manifest (`WORKFLOW.yaml`/`.json`) |
| Container / target | `.claude/workflows/<name>.<ext>` | SkillMeat-managed |
| Add command | `skillmeat add orchestration <local-file>` | managed via `workflow` commands |
| Source | local path only (not fetched from GitHub) | — |

**Add + deploy**

```bash
# Add to collection (extension validated: must be .js/.ts/.py/.sh)
skillmeat add orchestration .claude/workflows/execute-plan.js --yes
#   optional: --name my-flow to override the artifact name

# Deploy to a project — lands at .claude/workflows/execute-plan.js (extension preserved)
skillmeat deploy execute-plan --project .
```

Deployment preserves the script's file extension automatically (the engine copies the source
suffix when the resolved destination has none), so `execute-plan.js` deploys as
`execute-plan.js`, not an extensionless file.

**Plan / permission templates**: use the same templates below, with `Type: orchestration` and
`Target: .claude/workflows/<name>.<ext>`.
...
Type: <skill|command|agent|hook|mcp|orchestration|workflow>
Target: .claude/<type>s/<artifact-name>/   # orchestration → .claude/workflows/<name>.<ext>
