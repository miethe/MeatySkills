...
| "What's deployed here?" | `skillmeat status --project .` or inspect `.claude/.skillmeat-deployed.toml` | Project deployment status |
| "Tell me about X" | `skillmeat show <name>` | Artifact details |
| "Check for upstream updates" | `skillmeat status --project .` or `skillmeat update <name> --strategy upstream` | Status report / explicit upstream pull |
| "Check project drift" | `skillmeat sync-check . --format json` | Project-vs-collection drift |
| "Capture local artifact edits" | `skillmeat sync-pull . --strategy overwrite --no-interactive --refresh-metadata` | Project-to-collection capture |
| "Refresh deployed artifacts from collection" | `skillmeat deploy <name> --type <type> --project . --overwrite --non-interactive --no-recipe-preview` | Collection-to-project refresh |
...
```bash
# Deployment status for current project
skillmeat status --project .

# Drift between project deployment metadata and collection
skillmeat sync-check . --format json

# Deployed artifact names live in project metadata
python3 - <<'PY'
import tomllib
with open(".claude/.skillmeat-deployed.toml", "rb") as fh:
    for row in tomllib.load(fh).get("deployed", []):
        print(row["artifact_name"], row["artifact_type"], row.get("deployment_profile_id", ""))
PY
```
...
### Check for Updates and Drift

**User Says**: "Check for updates", "Is X up to date?", "What's new?"

```bash
# Check project deployment status and upstream availability
skillmeat status --project .

# Check project-vs-collection drift
skillmeat sync-check . --format json

# Diff explicit files/artifact versions when needed
skillmeat diff artifact <artifact-name>
```
...
### Capture Local Artifact Edits

**User Says**: "Capture changes I made to deployed artifacts", "Sync local project edits back"

```bash
# Preview project-to-collection capture
skillmeat sync-pull . --dry-run --format json

# Capture tracked project edits back to collection
skillmeat sync-pull . \
  --strategy overwrite \
  --no-interactive \
  --auto-link \
...
### Refresh Project from Collection

**User Says**: "Pull collection updates into this project", "Refresh deployed artifacts"

```bash
# Refresh one tracked artifact from the active collection
skillmeat deploy <artifact-name> \
  --type <skill|command|agent|orchestration> \
  --project . \
  --overwrite \
  --non-interactive \
  --no-recipe-preview \
...
# If deployment metadata has a profile, preserve it
skillmeat deploy <artifact-name> --type skill --project . --profile claude_code --overwrite --non-interactive --no-recipe-preview
```
...
### Update Collection from Upstream

Do this only when the user explicitly wants upstream source updates. Hermes wake-cycle automation
must not run this automatically; it should use `status` to flag upstream changes for review.

```bash
# Update a single collection artifact from upstream
skillmeat update <artifact-name> --strategy upstream

# Resolve local-vs-upstream conflicts interactively
skillmeat update <artifact-name> --strategy prompt
```

### Hermes Wake-Cycle Pattern

```bash
# Preview only
infra/agentic-node/hermes/scripts/hermes-skillmeat-cycle-sync.sh --check

# Default wake hook behavior
infra/agentic-node/hermes/scripts/hermes-skillmeat-cycle-sync.sh
```

The Hermes hook reads `.claude/.skillmeat-deployed.toml`, captures tracked local edits via
`sync-pull`, redeploys only artifacts already deployed into the project, validates SOUL rendering,
creates a SkillMeat memory candidate when a SOUL hash changes, and uses `status` to flag upstream
updates. It does not auto-deploy every new collection artifact and does not call `skillmeat update`.
