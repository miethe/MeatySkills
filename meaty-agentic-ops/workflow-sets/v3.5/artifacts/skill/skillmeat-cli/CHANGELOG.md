...
- Updated artifact management workflow for the live SkillMeat 0.60 CLI surface:
  `status`, `sync-check`, `sync-pull`, `deploy --overwrite`, and explicit `update`.
- Added Hermes wake-cycle guidance: cycle automation captures tracked `.claude` edits, refreshes
  only already-deployed artifacts from collection, validates SOUL rendering, creates SkillMeat memory
  candidates for SOUL hash changes, and flags upstream updates without automatically running
  upstream `skillmeat update`.
