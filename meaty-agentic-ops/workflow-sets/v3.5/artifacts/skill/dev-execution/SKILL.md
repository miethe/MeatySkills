...
`wave_plan.phases[]`; the subscription-side default when unset is Sonnet 5. Separately, a plan-level
`wave_plan.orchestrator_model` sets the **orchestration-loop** model (the session / phase main-loop
driver; default that cascades to all phases), with `wave_plan.phases[].orchestrator_model` overrides
for the rare Tier 3 / large-Tier-2, high-novelty or open-OQ phase. This is distinct from the delegate
`model` above (which sizes the implementer `Task()` calls inside a phase); model IDs use the registry
short form (`opus-5`, `sonnet-5`, `fable-5`).
...
The resolved per-phase `orchestrator_model` (from `wave_plan.orchestrator_model` + phase overrides) is echoed at phase startup and surfaced in the `/plan:plan-feature` execution handoff (`Execute: /dev:execute-plan … — orchestrator: **<model>** …`). It is **advisory** — it tells the operator which model to launch the session/phase under; the workflow cannot switch its own main-loop model mid-run, and it does not change the delegate `model=` passed to `Task()`.
...
The execution flow re-runs `itt sync import <file> --apply --tree <tree>` at four status hook points
to propagate task/phase status to bound IntentTree nodes. **This is on by default** (AOS
...
| `hooks/update-dossier.sh` | **Each phase boundary** (`modes/phase-execution.md` §5.2b, `modes/plan-execution.md` §3c-dossier / §7) | Re-renders + re-validates the manifest the phase-closing agent just wrote a stage into. |

Your job at a phase close is the **stage delta** — write this phase's `narrative` / `outcome` /
`decisions` / `evidence` into the manifest as you write the completion note (a decision point, so no
model call sits on the render path), then let the hook render. If no manifest exists the hooks
no-op silently: the feature was planned without a seed, which is not an error to chase mid-run.
Spec: `docs/skill-development/delivery-dossier/spec.md`.
