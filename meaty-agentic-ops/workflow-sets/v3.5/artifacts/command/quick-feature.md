...
> **Git workflow.** For multi-step or >1-file work, follow the canonical [git worktree + PR protocol](../../skills/dev-execution/git-worktree-pr-protocol.md): a **worktree** under `.claude/worktrees/<slug>`, the **parent branch** (HEAD at run start) as the PR base, a PR to the **parent branch**, and **squash-merge on approval / in-prompt override**. A true single-file change the user scoped as "just commit it" may commit in place (the protocol's trivial-single-file exception).
>
> **Model routing.** Subscription-side execution defaults to **Sonnet 5** (`claude-sonnet-5`) — **Opus 5** for spine, **`xhigh`** for the hardest work; offload bounded, contract-clear work to **ICA Sonnet 5** (`claude-sonnet-5[1m]`, free-to-us; 4.6[1m]/Haiku for cheap fan-out) behind the quality gate, never Mode-D work. Policy: [`MODEL-ROUTING`](../../../docs/agentic-operator/MODEL-ROUTING.md).
...
### 7. Next Actions Table (standard close)

End the response with the **Next Actions table** — spec: [.claude/skills/dev-execution/references/next-actions-table.md]. Emit a row for any follow-up or risk the change surfaced; otherwise emit the one-line empty state (`Complete. No follow-up actions; work is merged.`). The table is a standing close, not optional.
