...
6. **After the plan is approved**, emit the execution handoff and run the plan via `/dev:execute-plan`. Do not start execution here — this command is planning-only. The handoff names the orchestration-loop model per the `wave_plan.orchestrator_model` convention (plan-level default; `wave_plan.phases[].orchestrator_model` overrides a single phase — distinct from the delegate `phases[].model`). Emit it verbatim:
   ...
   Model IDs use the registry short form (`opus-5`, `sonnet-5`, `fable-5`). List the `(per-phase overrides: …)` clause only when at least one phase carries its own `orchestrator_model`; omit it otherwise.
