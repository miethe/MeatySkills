...
5. Finalize the brief:
   ...
   python .claude/skills/artifact-tracking/scripts/manage-exploration-status.py \
     ...
     --status finalized
   ```
6. **Recommended — produce a `readiness` delivery-report** when a stakeholder-facing go/no-go artifact
   is wanted (an investment decision, a shared review). Invoke `Skill("delivery-report")`, route
   `readiness`, subject = the idea slug; source its content from the finalized `feasibility_brief`
   (which stays canonical — the report is the shareable presentation of the verdict + cited evidence).
   Recommended, **not blocking**; skip for a low-stakes solo decision. See the planning skill's
   "Status & Readiness Reporting" section.
...
Then close the response with the **Next Actions table** — spec: [.claude/skills/dev-execution/references/next-actions-table.md]. The verdict becomes the row: `go` → `/plan:plan-feature --tier=[N]` (Target = the feasibility-brief path); `conditional` → a `deferred` row carrying the `defer-until:` trigger; `no-go` → a `human decision` row noting the archive. When a `readiness` delivery-report was produced (step 6), keep the table front-and-center in the response with the report path listed as an artifact.
