...
status: active
created: 2026-06-01
updated: 2026-08-03  # reviewer-gate registered active; the mandatory gate as a schema'd stage
owner: nick
related_documents:
...
| `reviewer-gate` | `.claude/specs/workflows/reviewer-gate-workflow-spec.md` | `.claude/workflows/reviewer-gate.js` | `active` | nick | P8 | The mandatory reviewer gate as a schema'd stage, for every gate NOT already inside `execute-plan`/`execute-contract` — Tier 0 close, scaffold close, plan-level whole-tree pass, milestone gate, fresh-context re-pass. Runs 1–2 edit-less lenses in `parallel`, each forced through `VERDICT_SCHEMA`; a null/throwing/unmapped lens becomes an explicit `verdict_source: 'gate_failure'` verdict rather than an empty pass or a phantom rejection (`gate_ran` separates *did not run* from *said no*). No fix loop by design — the caller owns the budget. No wall-clock timeout: `agent()` exposes no deadline, and the spec says so rather than implying one. Never calls `workflow()`, so a top-level workflow can nest it. |
